...
> 刚才说的 Mutex（互斥锁），抢不到锁就去睡觉（交出 CPU 控制权，也就是操作系统管理的 waitqueue 进程挂起休眠），因为睡觉和被唤醒的开销很大（需要操作系统进行上下文切换）。
> 那如果是极短时间就能干完的活呢？我们就用 Spinlock（自旋锁）。打工人不睡觉，就在门口死盯着（利用硬件提供的 CAS (Compare-And-Swap) 指令在一个死循环里疯狂重试），一旦锁解开就冲进去。这是一种“拿 CPU 烧电费换取极致响应速度”的做法。
